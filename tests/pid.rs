...
fn is_close<T: Float>(a: T, b: T, tolerance: Tolerances<T>) -> bool {
    ...
    return diff < tolerance.absolute.max(tolerance.relative * norm);
}
...
fn test_output_within_limits() {
    ...
    let mut output;
    for _ in 0..10 {
        (ctx, output) = pid.compute(&ctx, 100.0, 0.0, now + Duration::from_millis(200), None);
...
fn test_derivative_response() {
    ...
    // Initial step
    let (ctx, _) = pid.compute(&ctx, 0.0, 1.0, now + Duration::from_millis(100), None);

    // No change
    let (ctx, output2) = pid.compute(&ctx, 0.0, 1.0, now + Duration::from_millis(200), None);

    // Sudden change in input
    let (_, output3) = pid.compute(&ctx, 1.0, 1.0, now + Duration::from_millis(300), None);
...
fn test_hold_integration_prevents_integral_growth() {
    ...
    let (ctx1, output1) = pid.compute(&ctx, 0.0, 1.0, now + Duration::from_millis(100), None);
    let (_, output2) = pid.compute(&ctx1, 0.0, 1.0, now + Duration::from_millis(200), None);
...
fn test_initialize_on_reactivation() {
    ...
    let (_, output1) = pid.compute(&ctx, 0.0, 1.0, now + Duration::from_millis(100), None);

    ctx.set_activity_level(PidActivity::Active);
    let (ctx2, output2) = pid.compute(&ctx, 0.0, 1.0, now + Duration::from_millis(200), None);
...
fn test_simulink_open_loop_behavior_compliance() {
    ...
    for i in 0..1000usize {
        ...
        (ctx, output) = pid.compute(&ctx, 0.0, setpoint, timestamp, None);
        if i % 20 == 0 {
            assert!(is_close(
                output,
...
fn test_simulink_closed_loop_behavior_compliance() {
    ...
    for i in 0..1000usize {
        let timestamp = ctx.last_time() + Duration::from_millis(data::FIXED_STEP_SIZE_MS);
        (ctx, output) = pid.compute(&ctx, state, 1.0, timestamp, None);
        if i % 20 == 0 {
            assert!(is_close(
                state,
